...
func AuthRoutes(db *gorm.DB, router *gin.Engine) {
	...
	authorize.Use(middlewares.AuthMiddleware())
	{
		...
		authorize.POST("/api/reservations/checkout/:id", rbac.RBACMiddleware("create"), server.CheckOut)
		authorize.POST("/api/reservations/completed/:id", rbac.RBACMiddleware("create"), server.Completed)
